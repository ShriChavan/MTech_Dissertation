...
\title[Automating Web Performance Optimization]{Automating Web Performance Optimization: An Explainable Predictive-Prescriptive AI Framework with Cross-Dataset Generalizability}


\abstract{
Web application performance remains a persistent engineering challenge, yet existing tooling largely confines practitioners to manual, reactive diagnostics. This paper describes an integrated predictive--prescriptive--explainable AI framework that shifts the workflow from passive assessment to active, automated optimization. A primary dataset of 885 webpages, enriched with Core Web Vitals from the Google PageSpeed Insights API, is used to train four supervised classifiers within a leak-proof scikit-learn Pipeline. XGBoost achieves 90.60\% test accuracy (pipeline cross-validation: $90.96 \pm 1.40$\%), with superiority confirmed by Friedman, Wilcoxon, and McNemar tests. A Differential Evolution engine translates these predictions into domain-compliant, direction-aware feature prescriptions, achieving a 100\% success rate on batch optimization. To move explainability beyond routine SHAP and LIME reporting, we contribute four methodological advances: (i)~a fidelity-weighted multi-method consensus ranking, (ii)~a quantitative stability and robustness analysis of local explanations, (iii)~category-stratified feature importance with Kruskal--Wallis significance testing, and (iv)~hybrid counterfactual explanations that reveal feature influence and feature actionability to be fundamentally different constructs. External generalizability is established through a separate study on 8,000 HTTP Archive URLs, where a model trained on the 885-URL primary set transferred at 97.54\% accuracy. A controlled real-world experiment independently verifies the prescriptive output: implementing the framework's recommendations on a deployed test site reduced Largest Contentful Paint by 97.3\% and raised the Google PageSpeed score from 56 to 100. The combined evidence positions the framework as a practical, transparent, and empirically grounded tool for intelligent web performance engineering.
}
...
This paper addresses each of these gaps within a single, cohesive framework. Our contributions are:

\begin{enumerate}
    \item \textbf{A leak-proof predictive pipeline} using scikit-learn Pipeline objects that wrap scaling inside cross-validation folds. Four classifiers (SVM, Random Forest, Gradient Boosting, XGBoost) are compared via RandomizedSearchCV, and the best model is confirmed through Friedman, Wilcoxon, and McNemar significance tests.

    \item \textbf{An external generalizability study} on 8,000 real-world URLs drawn from the HTTP Archive, independently labelled using Google's published performance thresholds. Cross-dataset transfer accuracy reaches 97.54\%, and an ablation study quantifies the marginal contribution of engineered features.

    \item \textbf{A direction-aware prescriptive engine} based on Differential Evolution that respects domain constraints (metrics that should decrease are bounded above by their current value; metrics that should increase are bounded below). Batch evaluation on five slow websites yields a 100\% success rate with full domain compliance.

    \item \textbf{Four novel explainability contributions:} (a)~a fidelity-weighted consensus ranking across SHAP, LIME, and Permutation Importance; (b)~quantitative LIME stability analysis (Jaccard index) and SHAP perturbation sensitivity; (c)~category-stratified Kruskal--Wallis testing revealing that 9 of 22 features differ significantly in importance across website categories; (d)~hybrid counterfactual explanations demonstrating that although SHAP and counterfactual rankings share moderate overall agreement (SHAP--counterfactual rank correlation $\tau = 0.598$, $p < 0.001$), specific features diverge markedly in their influence vs.\ actionability, with composite timing metrics (\textit{tti}, \textit{tbt}, \textit{TBT\_TTI\_Ratio}) appearing far more frequently in counterfactuals than their SHAP importance implies.

    \item \textbf{A controlled real-world experiment} where prescriptions are implemented on a live-deployed test site and audited by the external Google PageSpeed Insights API, eliminating the circular validation common in prior work.

    \item \textbf{End-to-end reproducibility:} all pipelines use a fixed random seed (42), stratified splits, and exported model artefacts.
\end{enumerate}
...
\centering
\begin{tabular}{p{0.35\textwidth} p{0.30\textwidth} p{0.29\textwidth}}
\toprule
Feature & Formula & Rationale \\
\midrule
Size\_LoadTime\_Ratio & Page Size / Load Time & Efficiency metric \\
Total\_Time & Response Time + Load Time & Combined latency \\
Throughput\_ \newline ResponseTime\_Ratio & Throughput / Response Time & Network efficiency \\
...
We deployed the baseline (slow) and optimized (fast) pages on Render.com and had both audited by the external Google PageSpeed Insights API.
...
\midrule
PageSpeed Score & 56 / 100 & 100 / 100 & \textbf{+81.8\%} \\
...
The model correctly predicted ``slow'' for the baseline page and ``fast'' for the optimized page. The PageSpeed API confirmed both classifications independently.

\begin{figure}[htbp]
    \centering
    \includegraphics[width=0.8\linewidth]{figures/slowpageperformance.png}
    
    \vspace{0.5cm} 
   
    \includegraphics[width=0.8\linewidth]{figures/fastpageperformance.png}
    
    \caption{Google PageSpeed Insights results for the baseline deployment (score 56, top) and the optimised deployment (score 100, bottom), independently verifying the framework's prescriptions. All prescription targets were met or exceeded, with an average metric improvement of 94.5\%. Domain compliance was 92.9\%, confirming that the optimizer's suggestions are practically realisable.}
...
\begin{enumerate}
    ...
    \item \textbf{Real-World Validation:} A controlled deployment experiment, audited by the external Google PageSpeed API, confirms that prescriptions reduce LCP by 97.3\% and raise the PageSpeed score from 56 to 100, eliminating circular validation.
\end{enumerate}

\subsection{Future Work}

\textbf{Automated Code Refactoring:} The current system prescribes feature-level changes; future work could use large language models to generate the specific code modifications (e.g., image compression scripts, JavaScript bundle splitting configurations) needed to implement those changes.
